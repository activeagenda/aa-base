...
Could not find the file $emptySQLPath, which is needed for installing the initial database tables. The database has been created, but without tables. If you do not have an available empty.sql file, you can create the necessary tables with the s2a.php script.

First generate the 'mod' and 'modgt' modules:

\$php s2a.php -m mod,modgt

Then generate all available modules:

\$php s2a.php -t

This will probably cause several error messages due to the missing tables, but the dependencies will be fulfilled as the script continues to run. If there are remaining failed modules after the s2a.php script is finished, repeat until there are no more errors (success!) or the same errors occur on the same modules (a problem).

After the s2a.php generation job has finished successfully, re-run this script but pass the -u switch:

php s2a-install-db.php -u

This will prompt you to supply information for an initial Administrator user, which is required in order to log in to the application via the web server.\n");
}
...
print "Now, we will set up the adminstrator user. This should be a real person (it is possible to set up more administrators in the application, so it is not necessary to share passwords).\n\n";
