...
/**
 *  Fills in the XML Excel & Word 2003 form of file and sends the output to browser.
  */
function renderMSXML($recordID)
{
    ...
    $msxmlHeader= MSXML_PATH .'/'. $this->reportPage->filename.'/header.xml';
	if(!file_exists($msxmlHeader)){
        trigger_error("Cannot find source PDF file $xlsHeader.", E_USER_ERROR);
    }
	$msxmlSnipet= MSXML_PATH .'/'. $this->reportPage->filename.'/snipet.xml';
	if(!file_exists($msxmlSnipet)){
        trigger_error("Cannot find source PDF file $xlsSnipet.", E_USER_ERROR);
    }
	$msxmlFooter= MSXML_PATH .'/'. $this->reportPage->filename.'/footer.xml';
	if(!file_exists($msxmlFooter)){
        trigger_error("Cannot find source PDF file $xlsFooter.", E_USER_ERROR);
    }

    header( 'content-disposition: attachment; filename="'.$this->reportPage->filename.'_'.date( 'Y-m-d_H.i.s').'.xml"' );
...
function getValue(&$data)
{
    ...
    if('' == $this->conditionValue){
        ...
        if(false !== strpos($this->conditionValue, ':')){
            list($matchType, $conditionValue) = split(':', $this->conditionValue);
        } else {
        ...
        switch($matchType){
        case 'contains':
            $isMatched = in_array($conditionValue, split(', ', $rawValue));
            break;
        case 'equals':
        default:
