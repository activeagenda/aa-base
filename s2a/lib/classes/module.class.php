        ...
        if ("List" == $pScreenName){
            ...
            if(count($Screens)){
                foreach ($Screens as $Screen){
                    ...
                    } else if('listreportscreen'  == strtolower(get_class($Screen))){
            ...
            foreach ($Screens as $Screen){
                ...
                if ($pScreenName != $Screen->name){
                    if (("view" == $pRestriction && "viewscreen" == strtolower(get_class($Screen)))                         || ("view" != $pRestriction)){
...
function BuildViewScreenSection($section){
    ...
    function BuildEditScreen($ScreenName){
        ...
        if($this->AllowAddRecord){
            $output['/**clone_as_new**/'] = '$cloneAsNew = true;';
            $output['/**keep_add_new**/'] = '$keepAddNew = true;';
        }
...
//START 
		...
		foreach($Screen->Fields as $field){
    		...
    		$vString = $this->ModuleFields[$field->name]->validate;
				if ( 0 < strlen( $vString ) ){
					$normalize_functions .= 
				"\$data['{$field->name}'] = Normalize(\$data['{$field->name}'], '{$vString}');\n\t\t\t";
				}            
			}
        }
...
function &Factory($element, $moduleID)
{
    $field = new ListField ($element, $moduleID);
    return $field;
}
...
function &Factory(&$element, $moduleID, &$callerRef)
{
    ...
    $index =& new $element->type($element, $moduleID, $callerRef);
    return $index;
}
