    ...
    if (MDB2::isError($dbResult)) {
        ...
        if(count($omit_codes) == 0 || !in_array($code, $omit_codes)){
            ...
            if(preg_match($native_code_pattern, $dbResult->userinfo, $matches)){
                ...
                if(0 == $native_code){ //This can happen when "commit/release savepoint cannot be done changes are auto committed". Best to just ignore after raising a notice
...
function handleError($error_number, $error_string, $error_file, $error_line)
{
    ...
    if(defined('EXEC_STATE') && EXEC_STATE == 1){
        ...
        .'<br/>'
		.gettext("You can help us by %sfiling an error report%s, explaining what you were trying to do, and what you would have expected to see. Please be as specific as you can.");
    ...
    $date = date('r');
    $error_msg = "An error of type {$errortype[$error_number]} (# $error_number) happened to $happenedTo at $date.\n";
